...
const DropFileInput = props => {
    ...
    const SaveImagesLink = () => { 
        ...
        NewimageUrlsString.forEach(imgString => {
            imgUrls.push(JSON.parse(imgString))
        });
        // fusion des nouvelles valeurs et des anciennes valeurs
        const newArray = props.imageUrls.concat(imgUrls)
        // suppresion les duplications (transformation des objets en string,application de Set[un Set est une liste d'elements unique],retrannsformation de String en Obj)
        let uniqueImgUrls = [...new Set(newArray.map(JSON.stringify))].map(JSON.parse);
        // maj avec le setImages
         props.SetimageUrls(uniqueImgUrls)
