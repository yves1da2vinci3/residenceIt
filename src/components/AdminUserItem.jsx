import React, { useState } from 'react'
import {MdOutlineCake,MdOutlineMailOutline} from 'react-icons/md'
import {BsPinMapFill} from 'react-icons/bs'
function AdminUserItem({Email,birthDate,
  firstName,
  lastName, sex,
    School,  isValidate}) {
    const [isGirl,setGirl] = useState(false)
  return (
    <div className="w-full lg:w-1/3 p-4">
    <div className="relative p-6 bg-white shadow rounded">
      <div className='flex justify-center'>
    { sex=== 1 ?   <img src='https://us.123rf.com/450wm/apoev/apoev1903/apoev190300160/124274049-person-white-photo-placeholder-woman-silhouette-on-gray-background.jpg?ver=6' 
  className='h-32 w-32  rounded-full '
  />  :  <img src='data:image/png;base64,' 
  className='h-32 w-32  rounded-full '
  /> }
  </div>
      <h3 className="text-center font-medium">{lastName + " " + firstName }</h3>
      {/* <p className="mb-6 text-center text-sm text-gray-500">
        Marketing Manager
      </p> */}
    
      <p className="flex items-center mb-4 text-sm mt-5">
        <span className="inline-block mr-2">
          <MdOutlineCake className='text-indigo-600 text-current' />
        </span>
        <span>{birthDate}</span>
      </p>
      <p className="flex items-center mb-4 text-sm">
        <span className="inline-block mr-2">
          <BsPinMapFill className='text-indigo-600 text-current' />
        </span>
        <span>{School}</span>
      </p>
      <p className="flex items-center mb-6 text-sm">
        <span className="inline-block mr-2">
          <MdOutlineMailOutline  className='text-indigo-600 text-current'  /> 
        </span>
        <span>{Email}</span>
      </p>
      <div className="flex items-center mb-6 text-sm">
        <h1 className="inline-block mr-2">
         status :
        </h1>
        {  !isValidate  ?      
                    <span className="py-1 px-3 text-sm text-orange-700 font-heading font-medium bg-orange-200 rounded-full">
                      rejeté
                    </span>
                    :   
                    <span className="py-1 px-3 text-sm text-green-700 font-heading font-medium bg-green-200 rounded-full">
                     accepté
                    </span>
                   }
      </div>
      
    </div>
  </div>
  )
}

export default AdminUserItem